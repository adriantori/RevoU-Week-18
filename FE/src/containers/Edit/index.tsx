...
const Edit: React.FC = () => {
    ...
    const initialTask = category ? category.task : '';
    const initialPriority = category ? category.priority : '';
    const unformattedInitialDue = category ? category.due : '';
    const initialAmount = category ? category.amount : '';
    const initialDue = dayjs(unformattedInitialDue, 'DD/MM/YYYY').format('YYYY-MM-DD');
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        if (task && priority && due && amount) {
            ...
            try {
                const fetching = await fetch(BASE_URL + `/update/${id}`, {
                    method: 'PATCH',
                    headers: {
                ...
                showNotification('success', 'Task Edited', 'Task edited successfully');
                navigate('/');
            } catch (error) {
